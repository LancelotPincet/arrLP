...
Modules go in `src/arrlp/modules/`.
Routines go in `src/arrlp/routines/`.
...
- Public objects must be exposed in `arrlp/__init__.py`
...
```python
# RIGHT
from arrlp import MyObject

# WRONG — never import from internal paths
from arrlp.modules.myobject_LP.myobject import MyObject
```

Every public object must be registered in `arrlp/__init__.py`:

```python
sources = {
    "<name>": "arrlp.modules.<name>_LP.<name>",
}
```
