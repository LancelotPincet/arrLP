...
# Initializations
def ini_vol_greyopening(self, array, *, structure=None, footprint=None, **kwargs) :
    return dict(
...
def out_vol_greyopening(self, array, **kwargs) :
...
# Main function
vol_greyopening = FunctionArray(
    
    # Mandatory
    ndims = 3,
    cpu_function = _vol_greyopening,
    par_function = par_vol_greyopening,
    gpu_function = _vol_greyopening,
    out_function = out_vol_greyopening,
    ini_function = ini_vol_greyopening,
...
if __name__ == '__main__' :
    ...
    func = vol_greyopening

    # Arguments
    kwargs = dict(
        footprint=kernel(ndims=3, pixel=1, window=12),
    )
