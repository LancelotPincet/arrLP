#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Author        : Lancelot PINCET
# GitHub        : https://github.com/LancelotPincet



# %% Libraries
from arrlp import FunctionArray



# %% Function

# Initializations
def ini_img_correlate(self, array, *args, kernel, **kwargs) :
    return dict(
        kernel=self.xp.asarray(kernel),
    )

def out_img_correlate(self, array, *args, **kwargs) :
    return self.xp.empty_like(array)

def cpu_img_correlate(self, out, array, *args, kernel, mode='constant', **kwargs) :
    return self.ndimage.correlate(array, weights=kernel, axes=self.axes, output=out, mode=mode, **kwargs)

def par_img_correlate(self, array, *args, kernel, mode='constant', **kwargs) :
    return self.ndimage.correlate(array, weights=kernel, mode=mode, **kwargs)

def gpu_img_correlate(self, out, array, *args, kernel, mode='constant', **kwargs) :
    try :
        self.ndimage.correlate(array, weights=kernel, axes=self.axes, output=out, mode=mode, **kwargs)
        raise SyntaxError('Cupy is updated on correlate with axes ! please correct')
    except TypeError :
        return self.ndimage.correlate(array, weights=kernel, output=out, mode=mode, **kwargs)



# Main function
img_correlate = FunctionArray(
    
    # Mandatory
    ndims = 2,
    cpu_function = cpu_img_correlate,
    par_function = par_img_correlate,
    gpu_function = gpu_img_correlate,
    out_function = out_img_correlate,
    ini_function = ini_img_correlate,

    # Loops
    par_loop = True,
    gpu_loop = True, # For now True, once cupy is updated axes argument sould be added like in scipy
    use_joblib = True, # If True, arguments of parallel function should not have "out".

    # Performances
    remove_parallel = False,
    remove_cuda = False,

)



if __name__ == '__main__' :
    from arrlp import debug_array
    from arrlp import kernel
    func = img_correlate

    # Arguments
    kwargs = dict(
        kernel=kernel(ndims=2, pixel=1, sigma=3),
    )

    # Modes
    modes = { # list of dicts with kwargs
        "Reference": dict(cuda=False, parallel=False),
        "Parallel": dict(cuda=False, parallel=True),
        "Cuda": dict(cuda=True, parallel=False),
    }

    debug_array(func, modes, **kwargs)
